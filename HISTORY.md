## 1.3.0 rc 2 - 2026-08-29
- VLESS supervisor автоматически восстанавливает повреждённую блокировку с пустым, отсутствующим, некорректным или устаревшим PID, поэтому health-check, failover и изменяющие команды больше не остаются заблокированными после прерванного цикла или перезагрузки.
- Ошибка захвата или освобождения VLESS-блокировки теперь объясняет причину и безопасное действие вместо молчаливого завершения команды с кодом 1.
- Удаление VLESS-соединения и восстановление lock покрыты регрессионными тестами без вывода UUID, ключей и других секретных параметров.

## 1.3.0 rc 1 - 2026-07-26
- Telemetry теперь работает с базовым Entware `jq` без Oniguruma, а GNU `stat` устанавливается явной зависимостью `coreutils-stat`; обновление с beta9 не требует конфликтующей замены `jq` на `jq-full`.
- Локальный capability preflight до запуска sender отдельно называет отсутствующий или несовместимый пакет и больше не выдаёт такую ошибку за `invalid_payload`, повреждённый state или отказ Monium.
- Интерактивные setup и uninstall теперь одинаково принимают обычные переводы строк, окружающие пробелы и bracketed paste, отличают добровольную отмену от ошибочного ввода и не выводят управляющие символы обратно в терминал.
- `mors uninstall --purge --yes` теперь полностью удаляет Mors вместе с профилями VLESS, данными телеметрии и временными файлами, поэтому чистая повторная установка не наследует старые настройки.
- Полное удаление затрагивает только данные Mors и сохраняет общие файлы, созданные пользователем или другими пакетами.
...
- Автоматическое переключение выполняется через Routing API Xray без перезапуска data-plane; один SOCKS-порт и один Keenetic Proxy-интерфейс остаются стабильными.
- Добавлены безопасная политика fail-closed, настраиваемый прямой резерв, состояния `paused/degraded/unavailable` и восстановление последнего активного соединения после перезагрузки.
- VLESS-параметры перенесены в версионное защищённое хранилище; рабочий Xray JSON генерируется транзакционно и проверяется до активации.
- Supervisor и изменяющие CLI-команды сериализованы общим decision lock; install/update/uninstall защищены отдельным lifecycle lock; добавлены проверяемые rollback, транзакционный backup и синхронизация endpoint-исключений.
- Добавлены `mors vless status|check|list|events --json`, документированные health-коды, локальная история событий и watchdog supervisor.
- Команда `mors vless new` удалена в пользу однозначных `mors vless add` и `mors vless update`.
- Добавлены общая дизайн-система CLI, архитектурный контракт VLESS и runtime CI-проверка multi-outbound конфига и balancer override на поддерживаемых версиях Xray.
- GitHub Actions переведены на Node.js 24 и закреплены полными SHA актуальных стабильных релизов.
...
- Синхронизированы README, CLI-справка и зависимости пакета с поддержкой VLESS Reality через Xray/Proxy.
- Добавлена политика совместимости Xray: минимум 1.8.24, протестировано до 26.2.6; setup предупреждает о проблемах, а запуск VLESS блокирует слишком старую версию.
- Добавлена команда `mors vless version` для диагностики установленной версии Xray.
- В CI добавлена проверка конфигурации VLESS на минимальной и последней протестированной версиях Xray.
...
- Исправлена ошибка, связанная с проверкой ssr соединения (см. заявка #[171](https://github.com/qzeleza/kvas/issues/171))
