...
Перед первичным `mors setup` для VLESS установите в KeeneticOS системные
компоненты **Proxy client**, **Netfilter** и **Netfilter Add-ons**. В setup plan
появится управляемый интерфейс `Proxy21`; если его ещё нет, Mors создаст его
внутри подтверждённой транзакции. Первое VLESS-соединение добавляется уже после
успешного setup. До этого `Proxy21` работает fail-closed и не выпускает
выбранный трафик напрямую.

Стандартный оставшийся от Kvas интерфейс `Kvas-proxy-vless` на `Proxy21` setup
принимает на обслуживание без удаления самого интерфейса и связанных с ним
системных политик. При ошибке rollback возвращает прежнее описание. Чужой
`Proxy21` с другим описанием Mors не меняет: освободите этот идентификатор или
переименуйте операторский интерфейс до setup.
...
При выборе `dnscrypt` setup проверяет подготовленный конфиг до commit, затем
дожидается реального DNS-ответа сначала от DNSCrypt, потом от dnsmasq. При
ошибке исходные файлы и состояния сервисов восстанавливаются транзакционно.
