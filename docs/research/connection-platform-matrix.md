...
только aarch64; минимальная OS для Mors в целом и полная модельная матрица
испытаниями не установлены. Универсальный Rust-бинарник из `PKGARCH:=all`
...
| AArch64; Hero 5G KN-4110 | `aarch64-k3.10` → `aarch64-3.10` | `cortex-a53`, 64 bit, glibc 2.27, GCC 8.4.0 [E3] | O: aarch64-архив явно указан для KN-4110 [K1]; общий минимум OS для Mors неизвестен | S+F; единственная конфигурация канонического builder. Свежая сборка и запуск на модели в #59 не выполнялись |
...
| Shadowsocks | Entware `shadowsocks-libev-ss-redir`, `ss-local`, config; не штатный RCI VPN-тип | O: внешняя proxy-точка допускается [K15]; S+F: собственный ss-redir путь Mors; общий минимум OS ? | Отдельный legacy backend в vpn; это не VLESS/Proxy и не полный общий пул; #80/#97 |
