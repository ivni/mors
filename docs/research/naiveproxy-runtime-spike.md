...
Проход LAN/Proxy был ограничен изолированным опытом на NC-1913:
...
**Итог #63: runtime-исследование и проверка исправленного MIPSel ELF завершены.**
Исправленный кандидат прошёл A/B handshake и полный mixed load без ошибок;
исходный неизменённый ELF воспроизводит дефект. Общая production-поставка и
реализация координатора остаются отдельными задачами. Получены
...
Актуальная сводка — «Исправленный ELF: сборка и проверка» в конце документа;
...
Это срез **до** локализации дефекта и проверки исправленного ELF.
Актуальное решение находится в заключительном разделе документа.
...
На этом этапе оставалась проверка полного исправленного ELF со **штатным**
timeout и смешанной нагрузкой. Она выполнена в следующем разделе. Сам по себе
source patch или большой timeout не был бинарным исправлением. Неизменённый
upstream ELF по-прежнему содержит дефект; production toolchain/поставка
следуют отдельному контракту.
...
## Исправленный ELF: сборка и проверка 24.09.2026

**Результат: выявленная гонка idle cleanup исправлена и проверена на полном
MIPSel-бинарнике, исполнявшемся на Viva NC-1913 / 5.00.C.12.0-0.**
Успешная [сборка](https://github.com/ivni/mors/actions/runs/36016911546)
выполнена в одноразовом Ubuntu 22.04 runner, с двумя compile jobs, за 49 min 39 s.
Локальный Docker штатно не стартовал из-за прежней ошибки `dockerInference`;
его настройки не ремонтировались, контейнеры на ПК не создавались.

Происхождение:

- Naive source `3ba967e2d36cc133a896e81a36257ad4c6ea20f4` +
  [patch](naiveproxy-63/idle-handshake-created-at.patch), SHA-256
  `01b7d7d4356ce8d6b9d5367c1322d919f96201a8fd99d96b9a896a943d0c935f`.
- Исправленный ELF SHA-256:
  `06b41b41473d24bcd0716e6c37dd39e6b765fa3389de8646e910f7e92b0725d9`.
  Digest сверен после скачивания и на роутере перед запуском.
- ELF32 LE, o32/MIPS32r2, static; OpenWrt 24.10.0/GCC 13.3.0 sysroot,
  Clang `llvmorg-23-init-10931-g20b6ec66-11`.
  [Manifest](naiveproxy-63/build-20260924-manifest.json),
  [фактические flags](naiveproxy-63/build-20260924-args.gn),
  [readelf-отчёт](naiveproxy-63/build-20260924-elf.txt).
  В текстовой копии readelf удалены только конечные пробелы; manifest содержит
  digest исходного `elf.txt` из Actions artifact.
- Строка версии у обоих ELF — `naive 150.0.7871.63`; отличать исправленный
  кандидат от stock asset необходимо по digest, а не только по версии.

### Проверка поведения на исправленном бинарнике

| Опыт | Результат |
| --- | --- |
| Исходный ELF, штатный idle timeout 600 s, 10 s пауза в handshake | 12/14 успешных аутентификаций, 2 преждевременных закрытия |
| Исправленный ELF, тот же штатный timeout и тот же роутер | 14/14, без преждевременных закрытий |
| Исправленный ELF, отдельный отрицательный опыт с idle timeout 5 s | Настоящий idle handshake закрыт через 58.878 s с учётом минутного cleanup timer |
| UDP ASSOCIATE | Отклонён, SOCKS reply 7; capability осталась TCP-only |
| Отдельный профиль с недоверенным CA | curl 35 / HTTP 0; соседний здоровый профиль — HTTP 200, 262144 bytes |
| Остановка собственного CONNECT/HTTPS fixture | curl 35 / HTTP 0, без успешного обходного ответа |
| Восстановление того же fixture, новая TCP-сессия | Первый проверочный запрос: HTTP 200, 262144 bytes за 0.510 s |

Большой диагностический idle timeout в этих проверках не использовался.
Значение 5 s было только отдельной негативной проверкой. Основной A/B и
mixed load работали со штатными 600 s, без override в профилях.

### Полный mixed load исправленного ELF

Все [обезличенные данные](naiveproxy-63/results-20260924-patched.json)
сохранены. На роутере — пять исправленных Naive (`L=4 + candidate`) и один
Xray 26.2.6 с рабочим VLESS/TLS. Сервер и генератор — на Pi, путь к серверу
идёт через WAN тестового роутера и Wi-Fi Pi. Fixture применяет стандартный
непаддированный HTTP/2 CONNECT; внешним provider-сервером он не объявляется.

После 300 s idle выполнены ступени 1/8/32 workers по 300 s. Последние
начатые запросы завершены перед следующей ступенью: фактические load-фазы
304.004 / 304.836 / 307.809 s. Ответ 256 KiB, curl limit 64 KiB/s,
connect timeout 8 s и общий 20 s. Одновременно: один VLESS worker, DoH
и последовательная ротация четырёх standby probes. DNS wire response сверялся
целиком. У активного Naive включён приватный NetLog.

| Naive workers | Naive успех / отказ | VLESS успех / отказ | DoH успех / отказ | Standby успех / отказ | DoH max, s |
| --- | --- | --- | --- | --- | ---: |
| 1 | 72 / 0 | 71 / 0 | 141 / 0 | 16 / 0 | 0.594 |
| 8 | 571 / 0 | 72 / 0 | 122 / 0 | 16 / 0 | 1.691 |
| 32 | 1105 / 0 | 71 / 0 | 29 / 0 | 16 / 0 | 9.325 |

Всего 1748 Naive + 214 VLESS + 292 DoH + 48 standby = **2302 успешных
прикладных проверок, ноль ошибок**. Прирост счётчиков независимого origin
совпал: 2010 data requests и 292 DNS requests. Серверный error counter пуст.
HTTP 407 challenge учитывался отдельно и не подменял итог прикладной пробы.

| Фаза | Naive RSS sum max, kB | Xray RSS max, kB | FD max одного Naive | FD max пула | MemAvailable min, kB | CPU пула peak, % capacity |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Idle | 38504 | 29556 | 10 | 52 | 161228 | 0.12 |
| 1 worker | 39584 | 32104 | 12 | 56 | 157504 | 6.71 |
| 8 workers | 40192 | 32532 | 19 | 63 | 155432 | 21.60 |
| 32 workers | 51720 | 32624 | 43 | 86 | 142808 | 32.84 |

Samples примерно каждые 10 s, по 28 внутренних samples на фазу. При
сопоставлении часов применён сдвиг router→Pi +7 s (точность примерно 1 s),
по 10 s у границ фаз исключены. Часы системы не менялись. CPU — доля всей
machine capacity; RSS sum повторно считает shared pages, PSS недоступен.
RAM/FD guard и аварийный TTL не сработали. NetLog добавляет нагрузку: сравнение
throughput/CPU с прежним неинструментированным прогоном не является A/B-тестом
производительности самого patch. DoH max 9.325 s требует отдельного решения
о product deadlines/concurrency в #117; память сама по себе не задаёт лимит.

### Заключение по #63

Исправленный кандидат получил подтверждение устранения обнаруженного дефекта
и прохождения указанного mixed load на NC-1913. Исторические ошибки сохранены
в datasets; их не переписали в PASS. Исходный stock ELF не считается
исправленным. Production-поставка должна закреплять исправленный digest либо
проверенный эквивалент upstream fix; AArch64/BE и общий coordinator/DNS/drain
остаются отдельными платформенными/интеграционными задачами.

Недоступность управления внешним provider-сервером остаётся принятым
ограничением. Его stop/restore не заявляется. Исследование #63 выполнено в
согласованных границах; Mors package/release не выпускался.

Проверки репозитория: [QA](https://github.com/ivni/mors/actions/runs/36018042214)
успешна — static/ShellCheck/actionlint, **493 BATS**, Xray 1.8.24 и 26.2.6.
Routes/rules NC-1913 и маршруты Pi после опыта совпали с baseline,
`ip_forward=0` и MTU 1500 сохранены; owned процессы и listeners остановлены.
Приватные runtime-каталоги удалены на роутере, Pi и ПК, включая сертификатные
ключи, профили, сырые NetLog и временные бинарники. Сохранены только
обезличенные evidence и исходники проверок; системные настройки ПК не менялись.
