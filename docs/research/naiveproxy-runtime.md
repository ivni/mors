...
**Новые данные #63:** тот же AArch64 ELF на существующем Raspberry Pi 3
(`aarch64`, Linux `6.18.34+rpt-rpi-v8`) получил SIGILL уже на `--version`.
Gdb указал `sha1h` в `sha1_block_data_order_hw`; CPU flags этого host не
содержат `sha1`. Это host-опыт, не Keenetic AArch64, однако он опровергает
допуск только по ELF machine или имени asset `cortex-a53`. Нужен отдельный
ISA/crypto-capability gate или корректный software fallback сборки; точный
полный набор необходимых расширений этим одним отказом не установлен.
Подробности и cleanup находятся в [отчёте #63](naiveproxy-runtime-spike.md).
...
**Уточнение по runtime #63 от 22.09.2026:** эти переменные управляют
дополнительным Unix trust store, а не отключают встроенный Chrome Root Store.
В [повторе на NC-1913](naiveproxy-runtime-spike.md) NetLog реального профиля
подтвердил Chrome Root Store version 33 и `is_issued_by_known_root=true`
при file, содержащем только синтетический root. Контролируемая TLS-матрица
отклонила неизвестный CA, неверное имя, просроченный сертификат и неполную
цепочку; доверенный custom CA и полная цепочка прошли HTTPS nonce-проверку.
Пустые file+directory поэтому не обязаны ломать публичный сертификат.
#69/#81 должны учитывать встроенные roots как часть binary revision;
ограничение доверия только заданным bundle этими переменными не реализуется.
Это уточняет прежнюю трактовку изоляции, но не закрывает всю WAN/TLS матрицу.
...
Итог runtime-исследования #63 от 24.09.2026 сохранён в
[отчёте](naiveproxy-runtime-spike.md#итоговый-проход-и-заключение-63-24092026).
На NC-1913 измерен пул L=4+1 с рабочим VLESS/TLS и DoH. Диагностика
SOCKS/TLS обрывов воспроизвела ошибку upstream idle cleanup: до первой
записи timestamp равен нулю, поэтому минутный таймер закрывает новый handshake.
Точечный patch собран в полный MIPSel ELF и проверен на NC-1913: A/B
14/14 handshake против 12/14 у stock, mixed load — 2302 успешные пробы без
ошибок. Digest исправленного ELF и границы проверки приведены в
[финальном отчёте](naiveproxy-runtime-spike.md#исправленный-elf-сборка-и-проверка-24092026).
Production-поставка должна закрепить исправление; budgets требуют отдельного
решения, в том числе из-за DoH latency под высокой нагрузкой.
Завершение исследования не означает готовности runtime к выпуску.
