...
Целевая интеграция общего Rust-ядра описана в
[ADR-0001](adr/0001-connection-core-boundaries.md): таблица замен охватывает
§1.7, §3–4, §6–7, передачу единственного владельца, внешние revisions и
rollback с сохранением внешних правок. Она активируется только через gates ADR;
текущие runtime и правило поставки одного `all.ipk` этим документальным
изменением не заменяются.
Для NaiveProxy ADR §2.1–2.2 и §7 добавляет в целевые транзакции поколения
process/config/CA, совместное применение TCP-route, UDP guard и DNS,
readback и scoped restore; поставка runtime необязательна и зависит от ABI.
Незакрытые transport/package gates блокируют активацию этих изменений.
